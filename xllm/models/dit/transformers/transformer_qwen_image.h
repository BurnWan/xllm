...
class QwenDoubleStreamAttnProcessor2_0Impl : public torch::nn::Module {
 public:
  ...
  virtual std::tuple<torch::Tensor, torch::Tensor> forward(
    ...
    const bool regione_enabled = DiTCache::get_instance().is_regione_enabled();
    if (regione_enabled && DiTCache::get_instance().regione_should_store_kv()) {
      DiTCache::get_instance().regione_store_img_kv(
          DiTCache::get_instance().regione_current_block(),
          DiTCache::get_instance().regione_current_use_cfg(),
          img_key,
          img_value);
    } else if (regione_enabled &&
               DiTCache::get_instance().regione_should_patch_kv()) {
      std::tie(img_key, img_value) =
          DiTCache::get_instance().regione_patch_img_kv(
              DiTCache::get_instance().regione_current_block(),
              DiTCache::get_instance().regione_current_use_cfg(),
              img_key,
              img_value);
    ...
    if (regione_enabled) {
      img_query_freqs =
          DiTCache::get_instance().regione_is_partial_mode()
              ? DiTCache::get_instance().regione_gather_query_rope(img_freqs)
              : img_freqs;
      img_key_freqs = DiTCache::get_instance().regione_gather_key_rope(
          img_freqs, img_key.size(1));
    }
 ...
 public:
  ...
  torch::Tensor forward(
    ...
    const bool regione_partial_sp_mode =
        regione_enabled &&
        DiTCache::get_instance().regione_is_partial_sp_mode();
    if (::xllm::ParallelConfig::get_instance().sp_size() > 1) {
    ...
    if (regione_enabled) {
      DiTCache::get_instance().regione_prefetch_img_kv(
          0, use_cfg, new_hidden_states);
    }
    ...
    if (!use_step_cache) {
      for (int64_t index_block = 0; index_block < transformer_blocks_->size();
        ...
        if (regione_enabled) {
          DiTCache::get_instance().regione_set_current_block(
